  ...
  try {
    if (medias) {
      ...
      if (ticket.channel === "whatsapp" && isPrivate === "false") {
        ...
        if (ticket.channel === "facebook") {
          // ✅ CORRIGIDO: Usar o body processado (com variáveis substituídas) para salvar no banco
          const bodyToSave = sendResult?.processedBody || body;
          // ✅ CORRIGIDO: Passar um objeto vazio como primeiro parâmetro já que sendText retorna void
